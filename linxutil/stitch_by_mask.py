# Stitch Two Images by Mask (Feathered, MASK input)
# Save as: custom_nodes/stitch_by_mask_feathered.py
from __future__ import annotations
import torch
import torch.nn.functional as F

print("[StitchByMaskFeathered] loaded")
...
class StitchByMaskFeathered:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "image_a": ("IMAGE",),
                "image_b": ("IMAGE",),
                "mask":    ("MASK",),   # <-- real MASK type
                "invert_mask": ("BOOLEAN", {"default": False}),
                "opacity": ("FLOAT", {"default": 1.0, "min": 0.0, "max": 1.0, "step": 0.01}),
                "feather_radius": ("INT", {"default": 5, "min": 0, "max": 100, "step": 1}),
    ...
    def blend(self,
        ...
        if force_size:
            ...
            if not (a.shape[1:3] == b.shape[1:3] == m.shape[1:3]):
                raise ValueError(f"Size mismatch: A {a.shape[1:3]}  B {b.shape[1:3]}  M {m.shape[1:3]}")
...
NODE_CLASS_MAPPINGS = {
    "StitchByMaskFeathered": StitchByMaskFeathered,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "StitchByMaskFeathered": "Stitch Two Images by Mask (Feathered)",
}
